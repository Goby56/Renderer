...
public class Handler {
	...
	public static void world(MatrixStack stack) {
		...
		int light = LightmapTextureManager.pack(world.getLightLevel(LightType.BLOCK, new BlockPos(pos)), world.getLightLevel(LightType.SKY, new BlockPos(pos)));
		ob.draw(stack, new Matrix4f(), pos, light);
//		OutlineFramebuffer.useAndDraw(() -> Renderer3d.renderFilled(stack, Color.WHITE, new Vec3d(0, 300, 0), new Vec3d(5, 5, 5)), 1f, Color.GREEN, Color.BLACK);
	...
	public static void hud(MatrixStack matrices) {
		if (fr == null) {
		...
		fs.push();
		Renderer2d.renderEllipse(matrices, Color.RED, 30, 120, 10, 15, 20);
		Renderer2d.renderEllipseOutline(matrices, Color.RED, 70, 120, 30, 15, 1+(System.currentTimeMillis() % 5000) / 5000d * 10, 1+(1- (System.currentTimeMillis() % 5000) / 5000d) * 10, 20);
		String n = """
